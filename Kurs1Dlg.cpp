...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT_R, m_EdA);
	DDX_Control(pDX, IDC_EDIT_RK, m_EdB);
	DDX_Control(pDX, IDC_EDIT_H, m_EdC);
	DDX_Control(pDX, IDC_EDIT3, m_EdSpeed);
	DDX_Control(pDX, IDC_EDIT_H2, m_EdD);
	...
	DDX_Control(pDX, IDC_EDIT4, m_float);
	DDX_Control(pDX, IDC_EDIT5, Beg);
	DDX_Control(pDX, IDC_EDIT6, End);
}
...
BEGIN_MESSAGE_MAP(CKurs1Dlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_R, &CKurs1Dlg::OnEnChangeEditA)
	ON_EN_CHANGE(IDC_EDIT_RK, &CKurs1Dlg::OnEnChangeEditB)
	...
	ON_EN_CHANGE(IDC_EDIT_H, &CKurs1Dlg::OnEnChangeEditC)
	...
	ON_EN_CHANGE(IDC_EDIT_H2, &CKurs1Dlg::OnEnChangeEditD)
	ON_EN_CHANGE(IDC_EDIT4, &CKurs1Dlg::OnEnChangeEdit4)
	ON_EN_CHANGE(IDC_EDIT5, &CKurs1Dlg::OnEnChangeEditBegin)
	ON_EN_CHANGE(IDC_EDIT6, &CKurs1Dlg::OnEnChangeEditEnd)
END_MESSAGE_MAP()
...
BOOL CKurs1Dlg::OnInitDialog()
{
	...
	m_A = uA;
	CString strA;
	strA.Format(L"%f.0", m_A);
	m_EdA.SetWindowTextW(strA);*/
	m_EdSpeed.SetWindowTextW(L"50"); 
	m_EdA.SetWindowTextW(L"1");
	m_EdB.SetWindowTextW(L"1");
	m_EdC.SetWindowTextW(L"1");
	m_EdD.SetWindowTextW(L"1");
	m_float.SetWindowTextW(L"0.1");
	m_bInited = true;
...
void CKurs1Dlg::OnBnClickedOk()
{
	...
	AfxGetApp()->WriteProfileInt(L"Settings", L"ParamA", (UINT)m_A);
	CDialogEx::OnOK();
}


void CKurs1Dlg::Risovat(double A, double B, double C, double D,double beg, double end, double e)
{
	if (e > 0) {
		m_Calc.SetPar(A, B, C, D,beg,end, e);
...
{
	Risovat(m_A, m_B, m_C, m_D,m_Begin, m_End,m_e);
	pStatic = (CStatic*)GetDlgItem(IDC_STATIC8);
	CString strValue;
	strValue.Format(_T("%f ,количество итераций: %d"), m_Calc.findKor(), m_Calc.iter); // Convert the double value to a string
	m_Calc.iter = 0;
	pStatic->SetWindowText(strValue); // Set the text of the CStatic control
...
void CKurs1Dlg::OnEnChangeEditB()
{
	CString strA;
	m_EdB.GetWindowText(strA);
	m_B = _wtof(strA);
	OnBnClickedWindowTileVert();
}
...
void CKurs1Dlg::OnTimer(UINT_PTR nIDEvent)
{
	//m_Fi += .017;
...
void CKurs1Dlg::OnBnClickedSave()
{
...
void CKurs1Dlg::OnEnChangeEditC()
{
	CString strA;
	m_EdC.GetWindowText(strA);
...
void CKurs1Dlg::OnEnChangeEditD()
{
	CString strA;
	m_EdD.GetWindowText(strA);
...
void CKurs1Dlg::OnEnChangeEdit4()
{
	CString strA;
	m_float.GetWindowText(strA);
	m_e = _wtof(strA);
	if (((float)(int)log10(m_e) != log10(m_e))) {
		if (m_e == 0 || strA == "0." || strA == "0.0" || strA == "0.00") {
			strA = strA;
		}
		else {
			AfxMessageBox(_T("точность должна быть отрицательной степению 10 либо равна 1"), MB_OK | MB_ICONERROR);
			m_float.SetWindowTextW(L"0.1");
		}
	}
	OnBnClickedWindowTileVert();
...
void CKurs1Dlg::OnEnChangeEditBegin()
{
	CString strA;
	Beg.GetWindowText(strA);
...
void CKurs1Dlg::OnEnChangeEditEnd()
{
	CString strA;
	End.GetWindowText(strA);
